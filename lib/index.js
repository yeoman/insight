...
Insight.prototype.trackEvent = function (category, action, label, value) {
	if (this.optOut) {
		return;
	}
	...
	if (!category || !action) {
		throw new Error('event category and event action required');
	}

	// timestamp isn't unique enough since it can end up with duplicate entries
	this._queue[Date.now() + ' ' + category + action + label + value] = {
		category: category,
		action: action,
		label: label,
		value: value,
